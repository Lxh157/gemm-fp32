...
// full A tile: [BLOCK_M, BLOCK_K] = [32, 32]
...
// full B tile: [BLOCK_K, BLOCK_N] = [32, 64]
...
// split each K32 stage into two K16 subgroups
constexpr int SUB_K = WMMA_K;                                // 16

// A half-stage: [32, 16], each row has 16 / 8 = 2 chunks
constexpr int A_SUB_ROW_CHUNKS = SUB_K / CHUNK_HALF;         // 2
constexpr int A_SUB_NUM_CHUNKS = BLOCK_M * A_SUB_ROW_CHUNKS; // 64

// B half-stage: [16, 64], each row has 64 / 8 = 8 chunks
constexpr int B_SUB_ROW_CHUNKS = BLOCK_N / CHUNK_HALF;       // 8
constexpr int B_SUB_NUM_CHUNKS = SUB_K * B_SUB_ROW_CHUNKS;   // 128
...
// Load one K16 substage into the corresponding half of the K32 shared tile.
// subk = 0 or 16
__device__ __forceinline__ void load_stage_half_to_shared_cpasync_k32_split(
    const half* __restrict__ A,
    const half* __restrict__ B,
    half* __restrict__ smemA_stage,
    half* __restrict__ smemB_stage,
    int block_row, int block_col, int k0, int subk,
    int K, int N, int tid) {

    // A half-stage: [32, 16], two 16B chunks per row
    for (int chunk = tid; chunk < A_SUB_NUM_CHUNKS; chunk += THREADS_PER_BLOCK) {
        const int row = chunk / A_SUB_ROW_CHUNKS;          // 0..31
        const int chunk_in_row = chunk % A_SUB_ROW_CHUNKS; // 0..1
        const int col = subk + chunk_in_row * CHUNK_HALF;  // 0/8 or 16/24
    ...
    // B half-stage: [16, 64], eight 16B chunks per row
    for (int chunk = tid; chunk < B_SUB_NUM_CHUNKS; chunk += THREADS_PER_BLOCK) {
        const int row = chunk / B_SUB_ROW_CHUNKS;          // 0..15
        const int chunk_in_row = chunk % B_SUB_ROW_CHUNKS; // 0..7
        const int col = chunk_in_row * CHUNK_HALF;         // 0,8,...,56

        half* smem_dst = &smemB_stage[(subk + row) * SMEM_STRIDE_B + col];
        const half* gmem_src = &B[(k0 + subk + row) * N + (block_col + col)];

        cp_async_cg_16B(smem_dst, gmem_src);
    }
}

// Load one full K32 stage as two cp.async groups:
//   group 0 => K[0:16)
//   group 1 => K[16:32)
__device__ __forceinline__ void load_stage_to_shared_cpasync_k32_split(
    const half* __restrict__ A,
    const half* __restrict__ B,
    half* __restrict__ smemA_stage,
    half* __restrict__ smemB_stage,
    ...
    load_stage_half_to_shared_cpasync_k32_split(
        A, B,
        smemA_stage, smemB_stage,
        block_row, block_col, k0, 0,
        K, N, tid
    );
    cp_async_commit();
    ...
    load_stage_half_to_shared_cpasync_k32_split(
        A, B,
        smemA_stage, smemB_stage,
        block_row, block_col, k0, WMMA_K,
        K, N, tid
    );
    cp_async_commit();
}

__global__ void gemm_wmma_fp16acc_staged_cpasync_k32_split_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    // preload stage 0
    load_stage_to_shared_cpasync_k32_split(
        A, B,
    ...
    for (int k0 = 0; k0 < K; k0 += BLOCK_K) {
        ...
        // kk = 0 (consume first K16 substage)
        {
        ...
        // keep at most one async-copy group outstanding before consuming kk=16
        if (next_k0 < K) {
            cp_async_wait_group<1>();
        }

        // kk = 16 (consume second K16 substage)
        {
            wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> a_frag;
            wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> b_frag;
...
void launch_gemm_wmma_fp16acc_staged_cpasync_k32_split(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % 16 != 0 || N % 16 != 0 || K % 32 != 0) {
        std::fprintf(stderr,
            "gemm_wmma_fp16acc_staged_cpasync_k32_split: currently requires M,N to be multiples of 16 and K to be a multiple of 32. "
            "Got M=%d N=%d K=%d\n",
            M, N, K);
    ...
    gemm_wmma_fp16acc_staged_cpasync_k32_split_kernel<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
