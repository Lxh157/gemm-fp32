...
__global__ void gemm_wmma_fp16acc_staged_cpasync_k32_skewA8_B32_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
...
void launch_gemm_wmma_fp16acc_staged_cpasync_k32_skewA8_B32(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % 16 != 0 || N % 16 != 0 || K % 32 != 0) {
        std::fprintf(stderr,
            "gemm_wmma_fp16acc_staged_cpasync_k32_skewA8_B32: currently requires M,N to be multiples of 16 and K to be a multiple of 32. "
            "Got M=%d N=%d K=%d\n",
            M, N, K);
    ...
    gemm_wmma_fp16acc_staged_cpasync_k32_skewA8_B32_kernel<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
