...
// N-direction CTA grouping for L2-aware traversal
...
__device__ __forceinline__ void load_stage_to_shared_cpasync_k64_4x4_skew16_swizzle_n4(
...
__global__ void gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skew16_swizzle_n4_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    const int cta_linear = blockIdx.x;

    const int block_m  = cta_linear / (groups_n * CTA_SWIZZLE_N);
    const int rem      = cta_linear % (groups_n * CTA_SWIZZLE_N);
    const int group_n  = rem / CTA_SWIZZLE_N;
    const int local_n  = rem % CTA_SWIZZLE_N;
    const int block_n  = group_n * CTA_SWIZZLE_N + local_n;
    ...
    // preload stage 0
    load_stage_to_shared_cpasync_k64_4x4_skew16_swizzle_n4(
        A, B,
...
void launch_gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skew16_swizzle_n4(
    const half* dA,
    const half* dB,
    float* dC,
    int M,
    ...
    if (M % 16 != 0 || N % 16 != 0 || K % 64 != 0) {
        std::fprintf(
            stderr,
            "gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skew16_swizzle_n4: "
    ...
    const int num_block_n = N / BLOCK_N;
    const int groups_n    = (num_block_n + CTA_SWIZZLE_N - 1) / CTA_SWIZZLE_N;
    const int total_ctas  = num_block_m * groups_n * CTA_SWIZZLE_N;

    dim3 grid(total_ctas);

    gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skew16_swizzle_n4_kernel<<<grid, block, 0, stream>>>(
        dA, dB, dC, M, N, K);
