...
constexpr int WARPS_PER_BLOCK_N = 8;
constexpr int WARPS_PER_BLOCK   = WARPS_PER_BLOCK_M * WARPS_PER_BLOCK_N; // 32
constexpr int THREADS_PER_BLOCK = WARPS_PER_BLOCK * 32;                  // 1024

constexpr int BLOCK_M = WARPS_PER_BLOCK_M * WMMA_M; // 64
constexpr int BLOCK_N = WARPS_PER_BLOCK_N * WMMA_N; // 128
...
// B tile: [BLOCK_K, BLOCK_N] = [32, 128]
// each row has 64 / 8 = 8 chunks
constexpr int B_ROW_CHUNKS = BLOCK_N / CHUNK_HALF; // 16
constexpr int B_NUM_CHUNKS = BLOCK_K * B_ROW_CHUNKS; // 32 * 16 = 512
...
__device__ __forceinline__ void load_stage_to_shared_cpasync_k32_4x8_skew16(
    ...
    // A tile: [64, 32], sixteen 16B chunks per row
...
__global__ void gemm_wmma_fp16acc_staged_cpasync_k32_4x8_skew16(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    const int warp_n = warp_id % WARPS_PER_BLOCK_N; // 0..7
    ...
    // preload stage 0
    load_stage_to_shared_cpasync_k32_4x8_skew16(
        A, B,
...
void launch_gemm_wmma_fp16acc_staged_cpasync_k32_4x8_skew16(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % 16 != 0 || N % 16 != 0 || K % 32 != 0) {
        std::fprintf(stderr,
            "gemm_wmma_fp16acc_staged_cpasync_k32_4x8_skew16: currently requires M,N to be multiples of 16 and K to be a multiple of 32. "
            "Got M=%d N=%d K=%d\n",
            M, N, K);
    ...
    gemm_wmma_fp16acc_staged_cpasync_k32_4x8_skew16<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
