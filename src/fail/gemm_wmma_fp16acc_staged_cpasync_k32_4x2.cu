...
// block = 4 x 2 warps => 64 x 32 output tile
...
constexpr int BLOCK_N = WARPS_PER_BLOCK_N * WMMA_N; // 32
...
// B tile: [32, 32]
// each row has 32 / 8 = 4 chunks
constexpr int B_ROW_CHUNKS = BLOCK_N / CHUNK_HALF;   // 4
constexpr int B_NUM_CHUNKS = BLOCK_K * B_ROW_CHUNKS; // 32 * 4 = 128
...
__device__ __forceinline__ void load_stage_to_shared_cpasync_k32_4x2(
    ...
    // A tile: [64, 32], four 16B chunks per row
...
__global__ void gemm_wmma_fp16acc_staged_cpasync_k32_4x2_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    const int warp_n = warp_id % WARPS_PER_BLOCK_N; // 0..1
    ...
    // preload stage 0
    load_stage_to_shared_cpasync_k32_4x2(
        A, B,
...
void launch_gemm_wmma_fp16acc_staged_cpasync_k32_4x2(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % 16 != 0 || N % 16 != 0 || K % 32 != 0) {
        std::fprintf(stderr,
            "gemm_wmma_fp16acc_staged_cpasync_k32_4x2: currently requires M,N to be multiples of 16 and K to be a multiple of 32. "
            "Got M=%d N=%d K=%d\n",
            M, N, K);
    ...
    gemm_wmma_fp16acc_staged_cpasync_k32_4x2_kernel<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
