...
constexpr int SKEW_HALF_A = 16;
constexpr int SKEW_HALF_B = 32;
constexpr int SMEM_STRIDE_A = BLOCK_K + SKEW_HALF_A; // 80
constexpr int SMEM_STRIDE_B = BLOCK_N + SKEW_HALF_B; // 96
...
__global__ void gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skewA16_B32_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
...
void launch_gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skewA16_B32(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % 16 != 0 || N % 16 != 0 || K % 64 != 0) {
        std::fprintf(stderr,
            "gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skewA16_B32: currently requires M,N to be multiples of 16 and K to be a multiple of 64. "
            "Got M=%d N=%d K=%d\n",
            M, N, K);
    ...
    gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skewA16_B32_kernel<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
